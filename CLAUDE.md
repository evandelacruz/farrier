...
| [docs/using.md](docs/using.md) | Day-two work on a running instance: CI, secrets, more than one project, what differs from GitHub. Only what is different because it is Farrier — never a Forgejo tutorial. |
| [docs/security.md](docs/security.md) | What the operator is accepting, and what to do about each item. Consequences, not rationale. |
...
- **IDs are build-time vocabulary:** requirement IDs belong in comments, doc comments, `docs/`, commit messages, and PR bodies. Never in what an operator reads at runtime — an event detail, an error, a flag description. `UP-006` names a document the reader does not have; say what the ID stands for instead, and point at [operating.md](docs/operating.md) or [security.md](docs/security.md) when they need more. A test in `cmd/farrier` enforces this over `cmd/`, `internal/`, and `web/`.
- **Repo host:** GitHub for now; agents work in branches and land through PRs with review and green CI. "Farrier hosts a real project" is the named dogfooding milestone, run against a private project rather than this public repository; [spec.md](docs/spec.md) holds its cutover sequence and what counts as met — landing one pull request end to end on the new instance, then `backup` and `drill`. `import` is step 2 of that sequence, not the test. "Farrier hosts Farrier" is a later milestone, gated on porting the agent fleet off the GitHub API.
