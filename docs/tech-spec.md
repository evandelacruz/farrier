...
4. Create `GitStatePath`/`GiteaStatePath` (UP-004, below) and bind-mount them into the forgejo service.
5. Resolve the bundle's persisted ed25519 SSH host key (INIT-003, `state.KeySSHHostKey`/`state.KeySSHHostKeyPublic`) and write it into `GiteaStatePath` at the location `forge.RenderAppINI` configures as Forgejo's `SSH_SERVER_HOST_KEYS` (`forge.SSHHostKeyPath`, under `forge.DataPath`) — the same directory step 4 just bind-mounted, so no extra mount is needed — then `chown -R` it to the forgejo container's uid:gid (RSTR-004, spec.md "Identity" > "Key material": "SSH host keys, installed on every deploy including the first, so a fresh host and a restored one both present an unchanged identity"). Without this, Forgejo generates and keeps its own host key the first time it boots with none present, which is never what `state.KeystoreKeyExporter` captures into a backup (it always reads the keystore, not the running host) — this step is what makes the keystore's copy the one actually in use, on the instance's original host and not only after a restore.
6. Converge the host to that Compose definition (ORCH-002).
7. Wait for the forgejo container to accept `docker compose exec`, since `up -d` returning doesn't guarantee the entrypoint has finished.
8. Provision the first admin account (FORGE-002).
9. Wait for the caddy container to accept `docker compose exec` the same way, so `up` returns only once the forge is actually serving HTTPS.
...
- Steps 1, 7, and 9 are read-only probes.
- Step 2 always re-ships the current `app.ini` and re-derives its checksum. `docker compose up -d` (step 6) decides whether to recreate a service by diffing its resolved config — image, environment, volumes, labels — never the bytes of a file a bind mount happens to point at, so without the checksum a content-only `app.ini` change would ship to disk without the running container ever picking it up. Carrying the checksum as an environment variable puts that content into the diff `docker compose` already does.
...
- Step 4's directory creation and chown are idempotent (`mkdir -p`, `chown` leave existing contents untouched).
- Step 5 always writes the same persisted key back, unconditionally — the same content overwritten with itself is a no-op in effect, so re-running `up` never drifts from whatever the keystore holds.
- Step 6 is idempotent by construction: `Converge` always ships the full Compose definition and replaces the remote directory wholesale, so `docker compose up -d --remove-orphans` reconciles from scratch on every call.
- Step 8 treats "the admin account already exists" (Forgejo's `admin user create` failing on a duplicate username) as done, not a failure, and does not re-emit or reuse the fresh password it generated for that call — the account already has its original password, handed to the operator on the run that actually created it.
...
<RemoteDir>/state/gitea    → forgejo:/data/gitea          (SQLite database, LFS objects, ssh/ — the bundle's SSH host key, RSTR-004)
...
`deploy.GitStatePath`, `deploy.GiteaStatePath`, and `deploy.BlobsStatePath` are this layout's one spelling — `configureState`, `backup.BuildOptions` (pointing `state.SSHGitExporter` at the same root it captures from), and `restore.Restore` (RSTR-001, placing a snapshot's git data and database back at the same paths before `deploy.Up` ever starts the container that reads them) all call them rather than each rebuilding `"<RemoteDir>/state/..."` independently.
...
## Restoring an instance (`restore`, RSTR-001, RSTR-002, RSTR-004)

`internal/core/restore.Restore` rebuilds a complete, verified working instance on a fresh host from a snapshot and a bundle, as one CORE-002 job:

1. Resolve the named snapshot (or the newest one, `backup.LatestSnapshotKey`) and fetch it from the source destination (`backup.Fetch`).
2. Decrypt it (`backup.DecryptArchive`) and verify it — the same `backup.Verify` a snapshot already passed at capture time (BKUP-004), against the same completeness/checksum/cross-consistency checks (tech-spec.md "Snapshot verification"). A snapshot that fails verification is never restored past this point (CLAUDE.md "verification is load-bearing").
3. Install the snapshot's key material into the target keystore, mirroring `initialize.Run`'s write path: a name the target already resolves to the identical value is left alone (the common case — the operator still holds the same keystore the bundle has always used); a name the target resolves to something *different* is a loud refusal, naming the key; a name genuinely absent is written fresh, the disaster-recovery case STATE-004 exists for.
4. Place the snapshot's git data and database directly onto the host, at `deploy.GitStatePath`/`deploy.GiteaStatePath` — the same paths UP-004 pins forge state to — streamed over the SSH session via `RunStdin` (`orchestrate.Client`'s stdin-streaming counterpart to `Run`) rather than buffered in memory. Each repository's object archive is extracted first, then its ref archive on top: the ref archive was captured during BKUP-002's push hold, pinned to the same moment as the database snapshot, while the object archive (captured after the hold released) can carry a newer ref state from a push that landed in the gap — applying the ref archive last restores exactly the hold-time, database-consistent state. Every file this step writes is then `chown -R`'d to the uid:gid the forgejo container runs as (`deploy.ChownState`): the SSH session placing it generally isn't that user, and `configureState`'s own chown only ever touches each directory's top level.
5. Restore blobs into the target's `blob.Adapter` (STATE-003) — the inverse of `backup.Run`'s own blob capture, one `Put` per captured object.
6. Run `deploy.Up` (UP-001..004) against the state just placed, relayed onto the same job the way `backup.Backup` relays `backup.Run`'s steps, but against `pinnedBundle(opts.Bundle, manifest.ForgejoVersion)` rather than the target bundle unchanged (RSTR-002, spec.md "Version pinning"): a copy of the target bundle whose forge image is overridden to the snapshot manifest's `ForgejoVersion` — the pinned image ref `backup.BuildOptions` captured from the source bundle's own `Manifest.Images[forge.Service]` at backup time — with Compose re-rendered from that override via `orchestrate.Render`, the same call `init` uses to build a bundle's Compose in the first place. This is what makes restore boot the exact version the snapshot was captured from even when the target bundle's own `farrier.yaml` currently pins something else (e.g. an `upgrade` run against it since the snapshot was taken); Compose is re-rendered rather than the target bundle's already-rendered Compose file patched directly, since `deploy.Up` ships `b.Compose` to the host as-is. `deploy.Up`'s own admin-bootstrap step sees the restored database already has an admin account and treats that as done, the same as a repeat `up` (UP-003), rather than minting a new one. `deploy.Up`'s own `configureSSHHostKey` and `configureTLS` steps (both documented under "Deployment" above) are what actually complete RSTR-004: because step 3 above already installed the snapshot's SSH host key and TLS certificate into the target keystore — matching if the operator's keystore already held them, written fresh otherwise — `deploy.Up` resolves those same values and installs them onto the running service, so the restored instance presents exactly the identity clients' `known_hosts` and any certificate pinning already expect, not a freshly generated one.

This is RSTR-001, RSTR-002, and RSTR-004. Reusing `backup.Verify` already gives restore the specific, named refusal RSTR-003 describes, but RSTR-003 is its own ID with its own acceptance bar.
