...
- **A loopback address is not one of the options when CI is deployed.** The address becomes `ROOT_URL`, which is what the forge tells CI job containers to clone from, and it is what the colocated runner registers against and fetches artifacts through. Job containers run on networks of their own, so inside them a loopback address is the container itself: the runner never connects, the first workflow queues forever, and nothing in the event stream or the admin UI says why. `up` refuses a loopback address when it is deploying a colocated runner, naming an address the host actually answers on — the machine's LAN address or its tailnet address, both of which a container can reach. An instance with the colocated runner turned off has no such consumer, and loopback stays legitimate there.
