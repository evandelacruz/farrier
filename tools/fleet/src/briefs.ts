...
export function buildImplementerPrompt(
  ...
  return [
    ...
    "",
    "**Put it in the package's own doc comments, not in docs/tech-spec.md.**",
    "Explaining what your code does, why an order matters, or what a failure path",
    "guarantees belongs next to the code, where it cannot drift away from it.",
    "This codebase already writes rich package docs — match them.",
    "",
    "docs/tech-spec.md covers only what no single package owns: the repository's",
    "shape, on-disk and on-the-wire formats, the driver protocols third parties",
    "implement against, and operational targets. **Do not add a section for your",
    "requirement ID.** If your change alters one of those cross-cutting things —",
    "a format, a protocol, a target, the package layout — update that section.",
    "Otherwise tech-spec needs no edit at all, and adding one makes it a second",
    "copy of the code that goes stale the next time the code moves.",
    "",
    "docs/spec.md is different: if you change or discover a settled *decision*,",
    "that belongs there, and per CLAUDE.md it is Evan's call, not yours.",
    "",
    SLICING_BLOCK,
    "",
    QUALITY_BLOCK,
