...
// TestRestoreSucceedsFromBundleCopiedToAnotherDirectory extends
// deploy.TestUpSucceedsFromBundleCopiedToAnotherDirectory and
// backup.TestBackupSucceedsFromBundleCopiedToAnotherDirectory's proof to the
// `restore` core path (XCUT-001's remaining note): every driver Restore
...
// identity that decrypts the fetched snapshot -- must keep working once the
// bundle directory itself has been physically relocated after being saved.
//
// As with the up and backup proofs, the keystore and blob directories the
// relocated bundle's manifest names (absolute per the XCUT-001 fix) stay
// put throughout; only the bundle directory moves. That matches what
// XCUT-001 actually claims: every operation works from any machine holding
// the bundle *and* key access, not that key access travels with the bundle
// directory itself.
func TestRestoreSucceedsFromBundleCopiedToAnotherDirectory(t *testing.T) {
	keysDir := t.TempDir()
	...
	// Rebuilt exactly as internal/api/restore.go's runRestore builds them
	// in production: fresh drivers constructed from the relocated bundle's
	// own manifest, plus the age identity resolved back out through the
	...
	// The snapshot restore fetches and decrypts must be encrypted under the
	// same identity ResolveIdentity just resolved from the relocated
	// bundle's keystore, exactly as a real snapshot for this bundle would
	// be.
	source := buildSnapshotForIdentity(t, resolvedIdentity, []state.Remote{
	...
	job := events.NewJob()
	if err := Restore(ctx, job, opts); err != nil {
		t.Fatalf("Restore: %v", err)
	}

	evs := job.Events()
	if len(evs) == 0 {
		t.Fatal("Restore emitted no events")
	}
	if last := evs[len(evs)-1]; last.State != events.StateSucceeded {
		t.Errorf("job's terminal event state = %s, want succeeded", last.State)
	}
	...
	// deploy.Up ran as the last stage of Restore: the compose definition
	// was converged on the host.
	converges := host.commandsContaining("docker compose up -d --remove-orphans")
	if len(converges) != 1 {
		t.Fatalf("got %d converge commands, want 1: %v", len(converges), host.commands)
	}
}

// buildSnapshotForIdentity is buildSnapshot (restore_test.go), reimplemented
...
func buildSnapshotForIdentity(t *testing.T, identity *age.X25519Identity, repos []state.Remote, dbRepos [][2]string) blob.Adapter {
	t.Helper()
	destDir := t.TempDir()
...
// here (rather than shared with deploy's and backup's own copyDir /
// copyBundleDir) since both are unexported to their own packages.
