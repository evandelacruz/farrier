...
// keyName in, secret bytes out. The manifest's keystore DriverRef carries
// only a driver name and its non-secret config — a directory, a command,
// an executable to run — never the secret itself (CORE-001, KEY-003).
//
...
// Driver resolves one named piece of key material to its secret bytes.
// Implementations must never log, cache to disk, or otherwise persist a
// resolved secret anywhere outside memory (KEY-003).
type Driver interface {
	Resolve(ctx context.Context, keyName string) ([]byte, error)
}
